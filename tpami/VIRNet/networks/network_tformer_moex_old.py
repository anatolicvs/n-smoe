...
class MoE_old(Backbone[MoEConfig]):
    ...
    def extract_parameters(self, p: torch.Tensor, k: int, ch: int) -> Tuple[
        ...
        if self.kernel_type == KernelType.GAUSSIAN_CAUCHY:
            ...
            color_uncon = p[:, :, 14 * k : (14 + 9) * k].reshape(B, ch, k, 9)
            if self.clamp_alpha:
            ...
            c = None
            if ch == 3:
                color_uncon = p[:, :, 12 * k : 12 * k + 9 * k].reshape(B, ch, k, 9)
            else:
                color_uncon = None
        ...
        cov_matrix = self.cov_mat(scale_xy, theta_xy, scale_color, color_uncon, ch)
        if self.kernel_type == KernelType.GAUSSIAN_CAUCHY:
    ...
    def cov_mat(
        ...
        color_uncon: Optional[torch.Tensor],
        ch: int,
    ) -> torch.Tensor:
        R = self.ang_to_rot_mat(theta_xy)
        S = torch.diag_embed(scale_xy.clamp(min=self.scale_min))
        C_xy = R @ S @ S.transpose(-2, -1) @ R.transpose(-2, -1)

        if ch == 3 and color_uncon is not None:
            B_, c_, k_, dim_ = color_uncon.shape
            lower_tri = color_uncon.view(B_, c_, k_, 3, 3).clone()
            diag_vals = lower_tri.diagonal(dim1=-2, dim2=-1).clone()
            diag_pos = F.softplus(diag_vals) + self.min_diag
            idx = torch.arange(3, device=lower_tri.device)
            lower_tri[..., idx, idx] = diag_pos
            upper_idx = torch.triu_indices(3, 3, 1)
            lower_tri[..., upper_idx[0], upper_idx[1]] = 0.0
            C_rgb = lower_tri @ lower_tri.transpose(-2, -1)

            C_full = torch.zeros(*C_xy.shape[:-2], 5, 5, device=C_xy.device)
            C_full[..., :2, :2] = C_xy
            C_full[..., 2:, 2:] = C_rgb
        else:
            C_color = scale_color.squeeze(-1).squeeze(-1).clamp(min=self.min_diag**2)
            C_full = torch.zeros(*C_xy.shape[:-2], 3, 3, device=C_xy.device)
            C_full[..., :2, :2] = C_xy
            C_full[..., 2, 2] = C_color
    ...
    def gaussian_cauchy_kernel(
        ...
        c_e = (
            c.unsqueeze(-1)
            .unsqueeze(-1)
            .expand(-1, -1, -1, norm_x.shape[-2], norm_x.shape[-1])
        )
        diag = Sigma_inv[..., 0, 0].clamp(min=1e-8)
        denominator = c_e * diag
        C_csigma = 1.0 / (1.0 + norm_x**2 / denominator)
        a_e = alpha.unsqueeze(-1).unsqueeze(-1)
        return a_e * G_sigma + (1 - a_e) * C_csigma
    ...
    def forward_spatial(
        ...
        g_full = torch.cat([g, g_color], dim=-1).unsqueeze(0).unsqueeze(0).unsqueeze(0)

        mu_color = torch.zeros(B, ch, k, ch, device=params.device)
...
class MoE_____(Backbone[MoEConfig]):
    """
    Parameter Layout (3-ch, GAUSSIAN_CAUCHY => 14/k):
      0..k     => mu_x
      k..2k    => mu_y
      2k..4k   => scale_xy(2)
      4k..5k   => theta
      ...
      7k..8k   => c
      8k..11k  => color_mean(3)
      11k..14k => color_diag(3)
    For GAUSSIAN => 12/k, skipping alpha,c.
    Make sure 'p' has shape (B, ch, 14*k) or (B, ch, 12*k) if 3-ch.
    """
    ...
    def extract_parameters(self, p: torch.Tensor, k: int, ch: int) -> Tuple[
        torch.Tensor,  # mu_xy => (B,ch,k,2)
        torch.Tensor,  # cov_matrix => (B,ch,k,(2+ch),(2+ch))
        torch.Tensor,  # w => (B,ch,k)
        Optional[torch.Tensor],  # alpha => (B,ch,k) or None
        Optional[torch.Tensor],  # c => (B,ch,k) or None
        torch.Tensor,  # color_mean => (B,ch,k, 3)
    ]:
        B, ch_dim, param_dim = p.shape
        # param_dim should be 14*k or 12*k for 3-ch
        # or adapt indexing if single-channel

        # GAUSSIAN_CAUCHY => 14 per kernel
        # GAUSSIAN       => 12 per kernel
        if self.kernel_type == KernelType.GAUSSIAN_CAUCHY:
            mu_x = p[..., 0:k].reshape(B, ch, k, 1)
            mu_y = p[..., k : 2 * k].reshape(B, ch, k, 1)
            scale_xy = F.softplus(p[..., 2 * k : 4 * k].reshape(B, ch, k, 2)) + 1e-3
            ...
            color_mean = p[..., 8 * k : 11 * k].reshape(B, ch, k, 3)
            color_diag = F.softplus(p[..., 11 * k : 14 * k].reshape(B, ch, k, 3)) + 1e-3
            ...
            color_mean = p[..., 6 * k : 9 * k].reshape(B, ch, k, 3)
            color_diag = F.softplus(p[..., 9 * k : 12 * k].reshape(B, ch, k, 3)) + 1e-3

            alpha, c = None, None

        if self.clamp_scale:
            scale_xy = scale_xy.clamp(self.scale_min, self.scale_max)
            color_diag = color_diag.clamp(self.scale_min, self.scale_max)

        # combine mu_x, mu_y
        mu_xy = torch.cat([mu_x, mu_y], dim=-1)  # => (B,ch,k,2)
        # build the covariance
        cov_matrix = self.cov_mat(scale_xy, theta_xy, color_diag, ch)
        if self.kernel_type == KernelType.GAUSSIAN_CAUCHY:
            cov_matrix = cov_matrix * self.sharpening_factor
    ...
    def cov_mat(
        ...
        scale_xy: torch.Tensor,
        theta_xy: torch.Tensor,
        color_diag: torch.Tensor,
        ch: int,
    ) -> torch.Tensor:
        R = self.ang_to_rot_mat(theta_xy)
        S = torch.diag_embed(scale_xy)
        C_xy = R @ S @ S.transpose(-2, -1) @ R.transpose(-2, -1)

        if ch == 3:
            diag_mat = torch.diag_embed(color_diag)  # => (B,ch,k,3,3)
            C_full = torch.zeros(*C_xy.shape[:-2], 5, 5, device=C_xy.device)
            C_full[..., :2, :2] = C_xy
            C_full[..., 2:, 2:] = diag_mat
        else:
            diag_val = color_diag.squeeze(-1)  # => (B,ch,k)
            C_full = torch.zeros(*C_xy.shape[:-2], 3, 3, device=C_xy.device)
            C_full[..., :2, :2] = C_xy
            C_full[..., 2, 2] = diag_val
        return C_full
    ...
    def forward_spatial(
        ...
        clamped = eigvals.clamp(min=1e-6)
        cov_matrix_pd = (eigvecs * clamped.unsqueeze(-2)) @ eigvecs.transpose(-2, -1)
        ...
        # shift color => (B,ch,k,2+ch)
        if ch == 3:
            mu_full = torch.cat([mu_xy, color_mean], dim=-1)  # => (B,ch,k,5)
        else:
            mu_full = torch.cat([mu_xy, color_mean], dim=-1)  # => (B,ch,k,3)

        mu_full = mu_full.unsqueeze(3).unsqueeze(4)
        S = Sigma_inv.unsqueeze(3).unsqueeze(4)
        ...
        # Weighted sum => no divide by sum(ker). w is softmax => sum(w_i)=1
        ker = ker * w.view(B, ch, k, 1, 1)
        out = ker.sum(dim=2)

        # Sigmoid for final pixel => [0..1]
    ...
    def gaussian_cauchy_kernel(
        ...
        diag_val = Sigma_inv[..., 0, 0].clamp(min=1e-8).unsqueeze(-2).unsqueeze(-1)
        denominator = c_e * diag_val
        C_csigma = 1.0 / (1.0 + norm_x**2 / denominator)
        alpha_e = alpha.unsqueeze(-1).unsqueeze(-1)
        return alpha_e * G_sigma + (1 - alpha_e) * C_csigma
...
class MoE_______(Backbone[MoEConfig]):
    def __init__(self, cfg: MoEConfig):
        super().__init__(cfg)
        self.kernel = cfg.kernel
        self.sharpening_factor = cfg.sharpening_factor
    ...
    def forward_spatial(
        ...
        if ch == 3:
            mu_full = torch.cat([mu_xy, color_mean], dim=-1)
        else:
            mu_full = torch.cat([mu_xy, color_mean], dim=-1)
        mu_full = mu_full.unsqueeze(3).unsqueeze(4)
        S = Sigma_inv.unsqueeze(3).unsqueeze(4)
...
class MoE________(Backbone[MoEConfig]):
    def __init__(self, cfg: MoEConfig):
        super().__init__(cfg)
        self.kernel = cfg.kernel
        self.sharpening_factor = cfg.sharpening_factor
        self.kernel_type = cfg.kernel_type
        self.min_diag_param = nn.Parameter(torch.tensor(1e-08))
    ...
    def construct_lower_triangular(
        ...
        if size == 2:
            indices = self.tril_indices_2
        elif size == 3:
            indices = self.tril_indices_3
        else:
            raise ValueError("Only size=2 or size=3 is supported.")
        L = torch.zeros(B, ch, k, size, size, device=params.device, dtype=params.dtype)
        params_flat = params.view(-1, n)
        L_flat = L.view(-1, size, size)
        L_flat[:, indices[0], indices[1]] = params_flat
        L = L_flat.view(B, ch, k, size, size)
        return L
    ...
    def extract_parameters(self, p: torch.Tensor, k: int, ch: int) -> Tuple[
        ...
        if self.kernel_type == KernelType.GAUSSIAN_CAUCHY:
            mu_x = p[..., 0:k].reshape(B, ch, k, 1)
            mu_y = p[..., k : 2 * k].reshape(B, ch, k, 1)
            L_spatial_params = p[..., 2 * k : 5 * k].reshape(B, ch, k, 3)
            L_spatial = self.construct_lower_triangular(L_spatial_params, size=2)
            theta_xy = (p[..., 5 * k : 6 * k].reshape(B, ch, k) + torch.pi) % (
                2 * torch.pi
            ) - torch.pi
            w = F.softmax(p[..., 6 * k : 7 * k].reshape(B, ch, k), dim=-1)
            alpha = torch.sigmoid(p[..., 7 * k : 8 * k].reshape(B, ch, k))
            c = F.softplus(p[..., 8 * k : 9 * k].reshape(B, ch, k)) + F.softplus(
                self.min_diag_param
            )
            L_color_params = p[..., 9 * k : 15 * k].reshape(B, ch, k, 6)
            L_color = self.construct_lower_triangular(L_color_params, size=3)
            L_color = F.softplus(L_color) + F.softplus(self.min_diag_param)
            color_mean = p[..., 15 * k : 18 * k].reshape(B, ch, k, 3)
            mu_xy = torch.cat([mu_x, mu_y], dim=-1)
            ...
            w = F.softmax(p[..., 6 * k : 7 * k].reshape(B, ch, k), dim=-1)
            alpha, c = None, None
            L_color_params = p[..., 7 * k : 13 * k].reshape(B, ch, k, 6)
            L_color = self.construct_lower_triangular(L_color_params, size=3)
            L_color = F.softplus(L_color) + F.softplus(self.min_diag_param)
            color_mean = p[..., 13 * k : 16 * k].reshape(B, ch, k, 3)
            mu_xy = torch.cat([mu_x, mu_y], dim=-1)
            cov_matrix = self.cov_mat(L_spatial, theta_xy, L_color, ch)
            return mu_xy, cov_matrix, w, alpha, c, color_mean, L_color
    ...
    def forward_spatial(
        ...
        k = self.cfg.kernel
        expected_param_dim = (
            18 * k if self.cfg.kernel_type == KernelType.GAUSSIAN_CAUCHY else 17 * k
        )
        assert params.shape[2] == expected_param_dim
        mu_xy, cov_matrix, w, alpha, c, color_mean, L_color = self.extract_parameters(
            params, k, ch
        )
        cov_matrix = 0.5 * (cov_matrix + cov_matrix.transpose(-2, -1))
        try:
    ...
    def gaussian_cauchy_kernel(
        ...
        c_e = c.unsqueeze(-1).unsqueeze(-1).expand_as(norm_x)
        diag_val = Sigma_inv[..., 0, 0].clamp(min=1e-6)
        denominator = c_e * diag_val
...
class MoE(Backbone[MoEConfig]):
    ...
    def construct_lower_triangular(
        ...
        if size == 2:
            ...
            raise ValueError("Only size=2 or size=3 is supported.")
        L = torch.zeros(B, ch, k, size, size, device=params.device, dtype=params.dtype)
        params_flat = params.reshape(-1, n)
        L_flat = L.view(-1, size, size)
        L_flat[:, indices[0], indices[1]] = params_flat
        L = L_flat.view(B, ch, k, size, size)
        return L
