...
class GeneralTrainFloder(uData.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_hr = torch.from_numpy(im_hr.transpose([2, 0, 1])).type(
            torch.float32
        )  # c x h x w
        im_lr = torch.from_numpy(im_lr.transpose([2, 0, 1])).type(
            torch.float32
        )  # c x h x w
        im_blur = torch.from_numpy(im_blur.transpose([2, 0, 1])).type(
            torch.float32
        )  # c x h x w
...
class GeneralTest(uData.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.noise_type == "Gaussian":
            im_lr = im_blur + self.fixed_noise[
                :h,
                :w,
            ] * (2.55 / 255)
            im_lr = np.clip(im_lr, a_min=0.0, a_max=1.0)
        elif self.noise_type == "JPEG":
            im_noisy = im_blur + self.fixed_noise[
                :h,
                :w,
            ] * (2.55 / 255)
