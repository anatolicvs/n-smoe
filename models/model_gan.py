...
class ModelGAN(ModelBase):
    ...
    def load(self):
        ...
        if load_path_G is not None:
            ...
            self.load_network(
                load_path_G, self.netG, strict=self.opt_train["G_param_strict"]
            )
        ...
        if self.opt_train["E_decay"] > 0:
            if load_path_E is not None:
                ...
                self.load_network(
                    load_path_E, self.netE, strict=self.opt_train["E_param_strict"]
                )
            else:
    ...
    def define_loss(self):
        ...
        self.D_update_ratio = (
            self.opt_train["D_update_ratio"] if self.opt_train["D_update_ratio"] else 1
        )
        self.D_init_iters = (
    ...
    def optimize_parameters(self, current_step):
        ...
        if self.opt_train["gan_type"] in ["gan", "lsgan", "wgan", "softplusgan"]:
            ...
            pred_d_fake = self.netD(
                self.E.detach().clone()
            )  # 2) fake data, detach to avoid BP to G
            ...
            pred_d_fake = self.netD(
                self.E
            ).detach()  # 1) fake data, detach to avoid BP to G
