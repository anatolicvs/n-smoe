...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, load_path=None, md=4):
        """
        ...
        if load_path:
            ...
            logger.info(f"Flownet is loaded from: {load_path}.")
...
class FlowGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.model = PWCDCNet(load_path=path)
        self.divisor = 64
    ...
    def forward(self, im1, im2):
        ...
        # PWC accepts images in BGR space.
        im1 = F.interpolate(
            input=im1[:, [2, 1, 0], :, :],
            size=(H_, W_),
            mode="bilinear",
        ...
        # pdb.set_trace()
        flow = self.model(im1, im2) * 20.0
        flow = F.interpolate(flow, size=(H_, W_), mode="bilinear")

        # scale the flow back to the input size
        flow = resize_flow(flow, size_type="shape", sizes=(H, W))

        # flow = F.interpolate(input=flow, size=(H, W),
