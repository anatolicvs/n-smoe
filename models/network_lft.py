...
class LFT(nn.Module):
    def __init__(self, args):
        ...
        resizer_num_layers = (
            args["resizer_num_layers"] if "resizer_num_layers" in args else 4
        )
        altblock_layer_num = (
            args["altblock_layer_num"] if "altblock_layer_num" in args else 8
        )
    ...
    def _interpolate(self, x, angRes, scale_factor):
        ...
        x_upscale = (
            x_upscale.permute(0, 2, 4, 1, 3, 5)
            .contiguous()
            .view(B * angRes**2, 1, h, w)
        )
        ...
        x_upscale = x_upscale.view(
            B, angRes, angRes, 1, h * scale_factor, w * scale_factor
        )
        x_upscale = (
...
class PositionEncoding(nn.Module):
    ...
    def forward(self, x, dim: list, token_dim):
        ...
        assert (
            len(x.size()) == 5
        ), "the object of position encoding requires 5-dim tensor! "
        grid_dim = torch.linspace(
            0, self.token_dim - 1, self.token_dim, dtype=torch.float32
        )
        ...
        for index in range(len(dim)):
            ...
            pos_dim = (
                torch.linspace(0, length - 1, length, dtype=torch.float32).view(-1, 1)
                / grid_dim
            ).to(x.device)
...
class SpaTrans(nn.Module):
    def __init__(self, channels, angRes, MHSA_params):
        ...
        self.MLP = MLP(
            channels * self.kernel_field**2,
            hidden_features=self.spa_dim,
            out_features=self.spa_dim,
        )

        self.norm = nn.LayerNorm(self.spa_dim)
        self.attention = nn.MultiheadAttention(
            self.spa_dim, MHSA_params["num_heads"], MHSA_params["dropout"], bias=False
        )
    ...
    def gen_mask(h: int, w: int, k: int):
        ...
        for i in range(h):
            for j in range(w):
                ...
                temp[
                    max(0, i - k_left) : min(h, i + k_right),
                    max(0, j - k_left) : min(h, j + k_right),
                ] = 1
                atten_mask[i, j, :, :] = temp
    ...
    def SAI2Token(self, buffer):
        ...
        spa_token = F.unfold(
            buffer, kernel_size=self.kernel_field, padding=self.kernel_field // 2
        ).permute(2, 0, 1)
...
class AngTrans(nn.Module):
    def __init__(self, channels, angRes, MHSA_params):
        ...
        self.attention = nn.MultiheadAttention(
            self.ang_dim, MHSA_params["num_heads"], MHSA_params["dropout"], bias=False
        )
        nn.init.kaiming_uniform_(self.attention.in_proj_weight, a=math.sqrt(5))
        self.attention.out_proj.bias = None
