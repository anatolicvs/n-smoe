...
class Discriminator_VGG_96(nn.Module):
    def __init__(self, in_nc=3, base_nc=64, ac_type="BL"):
        ...
        # 48, 64
        conv2 = B.conv(
            base_nc, base_nc * 2, kernel_size=3, stride=1, mode="C" + ac_type
        )
        conv3 = B.conv(
            base_nc * 2, base_nc * 2, kernel_size=4, stride=2, mode="C" + ac_type
        )
        # 24, 128
        conv4 = B.conv(
            base_nc * 2, base_nc * 4, kernel_size=3, stride=1, mode="C" + ac_type
        )
        conv5 = B.conv(
            base_nc * 4, base_nc * 4, kernel_size=4, stride=2, mode="C" + ac_type
        )
        # 12, 256
        conv6 = B.conv(
            base_nc * 4, base_nc * 8, kernel_size=3, stride=1, mode="C" + ac_type
        )
        conv7 = B.conv(
            base_nc * 8, base_nc * 8, kernel_size=4, stride=2, mode="C" + ac_type
        )
        # 6, 512
        conv8 = B.conv(
            base_nc * 8, base_nc * 8, kernel_size=3, stride=1, mode="C" + ac_type
        )
        conv9 = B.conv(
            base_nc * 8, base_nc * 8, kernel_size=4, stride=2, mode="C" + ac_type
        )
