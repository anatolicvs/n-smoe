...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        mlp_hidden_dim = int(dim * mlp_ratio[0])
...
class SwinAttnScale(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False,  latent_dim=10, **kwargs):
        super().__init__()
...
def test_swin_transformer_with_forward_pool():
    input_tensor = torch.randn(1, 3, 120, 120)
    model = SwinAttnScale(in_chans=3, latent_dim=10, mlp_ratio=4)
    output = model(input_tensor)
    
    print("Output shape:", output.shape)
    assert output.shape == (1, 3, 10), "Output shape is incorrect"

test_swin_transformer_with_forward_pool()