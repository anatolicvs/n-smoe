...
#     # def gaussian_cauchy_kernel_(self, x, mu, Sigma_inv, alpha, c):
#     #     d = x - mu
#     #     x1 = d.unsqueeze(-2)
...
#     def gauusian_kernel(self, x, mu, Sigma_inv):
#         x_sub_mu = x - mu
#         x_sub_mu_t = x_sub_mu.unsqueeze(-1)
#         exp_terms = -0.5 * torch.einsum(
#             "bnkhwli,bnkhwlj,bnkhwmi->bnkhw",
...
#     #     g = self.grid(height, width, device)if ch == 1:
#             scale_color = F.softplus(p[:, :, 8 * k : 9 * k].reshape(B, ch, k, 1))
#         elif ch == 3:
#             scale_cor = torch.zeros(height, width, 3, device=device)
#     #     g_full = (
