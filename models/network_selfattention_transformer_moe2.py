...
class MullerResizer(nn.Module):
    def __init__(self, d_in=3, base_resize_method='bilinear', kernel_size=5, stddev=1.0, num_layers=2, avg_pool=False, init_weights=None, dtype=torch.float32):
        ...
        self.gauss_kernel = self.create_gauss_kernel(kernel_size, stddev)

    def create_gauss_kernel(self, kernel_size, stddev):
        t = torch.arange(kernel_size, dtype=self.dtype) - (kernel_size - 1) / 2
        gauss_kernel = torch.exp(-t.pow(2) / (2 * stddev**2))
        gauss_kernel /= gauss_kernel.sum()
        gauss_kernel = gauss_kernel.view(
            1, 1, kernel_size, 1) * gauss_kernel.view(1, 1, 1, kernel_size)
        gauss_kernel = gauss_kernel.repeat(self.d_in, 1, 1, 1)
        return gauss_kernel
    ...
    def _apply_gauss_blur(self, x):
        ...
        gauss_kernel = self.gauss_kernel.to(x.device)
        return F.conv2d(x, gauss_kernel, groups=self.d_in)
    ...
    def forward(self, x, size):
        ...
        net = F.interpolate(
            x, size=size, mode=self.interpolation_method, align_corners=False)

        for weight, bias in zip(self.weights, self.biases):
            blurred = self._apply_gauss_blur(x)
            residual = blurred - x
            resized_residual = F.interpolate(
                residual, size=size, mode=self.interpolation_method, align_corners=False)
...
class BackboneDino(Backbone[BackboneDinoCfg]):
    def __init__(self, cfg: BackboneDinoCfg, d_in: int, d_out: int) -> None:
        super().__init__(cfg)
        try:
            self.dino = load("facebookresearch/dino:main", cfg.model)
    ...
    def get_dino_feature_dim(self) -> int:
        feature_dims = {
            "dino_vits16": 384,
            "dino_vits8": 384,
            "dino_vitb16": 768,
        ...
        return feature_dims.get(self.cfg.model, 768)

    def _configure_dino_patch_embedding(self, d_in: int) -> None:
    ...
    def _create_mlp(self, input_dim: int, output_dim: int) -> nn.Sequential:
    ...
    def forward(self, context: BatchedViews) -> torch.Tensor:
        ...
        combined_features = resnet_features + local_tokens + global_token
...
if __name__ == "__main__":
    ...
    encoder_cfg = EncoderConfig(
        ...
        backbone_cfg=BackboneDinoCfg(
            ...
            model="dino_vits8",
            backbone_cfg=BackboneResnetCfg(
                name="resnet", model="resnet50", num_layers=2, use_first_pool=True)
        ),
