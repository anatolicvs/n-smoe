...
def pad_collate_fn(batch):
    ...
    if isinstance(first_elem, dict):
        ...
        padded_batch = {
            key: torch.stack(
                [
                    F.pad(item[key], (0, max_lengths[key] - item[key].size(0)))
                    for item in batch
                ]
            )
            for key in keys
        ...
        padded_batch = torch.stack(
            [F.pad(item, (0, max_length - item.size(0))) for item in batch]
        )
...
def get_m_image_paths(
    dataset_dirs: Union[str, List[str]], data_name: str = "ALL"
) -> List[str]:
...
def is_image_file(filename: str) -> bool:
    lower_filename = filename.lower()
    return any(lower_filename.endswith(ext) for ext in IMG_EXTENSIONS) or (
        ".gz" in lower_filename
        and ("4ch_es.nii" in lower_filename or "t1n" in lower_filename)
    )
...
#         return True
#     if lower_filename.endswith(".gz") and (
#         "4ch_es.nii" in lower_filename or "t1n" in lower_filename
#     ):
#         return True
...
def imssave(imgs, img_path):
    ...
    for i, img in enumerate(imgs):
        ...
        new_path = os.path.join(
            os.path.dirname(img_path), img_name + str("_{:04d}".format(i)) + ".png"
        )
        cv2.imwrite(new_path, img)


def split_imageset(
    original_dataroot, taget_dataroot, n_channels=3, p_size=512, p_overlap=96, p_max=800
):
    """
