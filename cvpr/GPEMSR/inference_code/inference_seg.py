...
if __name__ == "__main__":
    ...
    if cfg.DATA.shift_channels is None:
        ...
        assert (
            cfg.MODEL.output_nc == cfg.DATA.shift_channels
        ), "output_nc must be equal to shift_channels"
    ...
    out_path = os.path.join(
        "/GPEMSR-CREMI/GPEMSR/inference_code/Result", trained_model
    )  # output path
    ...
    if args.waterz:
        ...
        f.create_dataset(
            "main", data=segmentation, dtype=segmentation.dtype, compression="gzip"
        )
        f.close()
        ...
        print(
            "waterz: voi_split=%.6f, voi_merge=%.6f, voi_sum=%.6f, arand=%.6f"
    ...
    if args.show:
        ...
        for i in range(output_affs_prop.shape[1]):
            cat1 = np.concatenate(
                [
                    output_affs_prop[0, i],
                    output_affs_prop[1, i],
                    output_affs_prop[2, i],
                ],
                axis=1,
            )
