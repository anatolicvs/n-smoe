...
class UNet_PNI(nn.Module):  # deployed PNI model
    ...
    def __init__(
        ...
        self.up0 = upsampleBlock(
            filters2[5], filters2[4], (1, 2, 2), upsample_mode, init_mode=init_mode
        )
        if self.merge_mode == "add":
            self.cat0 = conv3dBlock(
                [0],
                [filters2[4]],
                bn_mode=[bn_mode],
                relu_mode=[relu_mode],
        ...
        self.up1 = upsampleBlock(
            filters2[4], filters2[3], (1, 2, 2), upsample_mode, init_mode=init_mode
        )
        if self.merge_mode == "add":
            self.cat1 = conv3dBlock(
                [0],
                [filters2[3]],
                bn_mode=[bn_mode],
                relu_mode=[relu_mode],
        ...
        self.up2 = upsampleBlock(
            filters2[3], filters2[2], (1, 2, 2), upsample_mode, init_mode=init_mode
        )
        if self.merge_mode == "add":
            self.cat2 = conv3dBlock(
                [0],
                [filters2[2]],
                bn_mode=[bn_mode],
                relu_mode=[relu_mode],
        ...
        self.up3 = upsampleBlock(
            filters2[2], filters2[1], (1, 2, 2), upsample_mode, init_mode=init_mode
        )
        if self.merge_mode == "add":
            self.cat3 = conv3dBlock(
                [0],
                [filters2[1]],
                bn_mode=[bn_mode],
                relu_mode=[relu_mode],
        ...
        self.out_put = conv3dBlock(
            [int(filters2[0])], [out_planes], [(1, 1, 1)], init_mode=init_mode
        )
...
class UNet_PNI_embedding(nn.Module):  # deployed PNI model
    ...
    def __init__(
        ...
        self.out_put = conv3dBlock(
            [int(filters2[0])], [self.emd], [(1, 1, 1)], init_mode=init_mode
        )
        # self.sigmoid = nn.Sigmoid()
...
class UNet_PNI_embedding_deep(nn.Module):  # deployed PNI model
    ...
    def __init__(
        ...
        self.out_put1 = conv3dBlock(
            [int(filters2[5])], [self.emd], [(1, 1, 1)], init_mode=init_mode
        )
        self.out_put2 = conv3dBlock(
            [int(filters2[4])], [self.emd], [(1, 1, 1)], init_mode=init_mode
        )
        self.out_put3 = conv3dBlock(
            [int(filters2[3])], [self.emd], [(1, 1, 1)], init_mode=init_mode
        )
        self.out_put4 = conv3dBlock(
            [int(filters2[2])], [self.emd], [(1, 1, 1)], init_mode=init_mode
        )
...
if __name__ == "__main__":
    ...
    model = UNet_PNI_embedding_deep(
        filters=[28, 36, 48, 64, 80], upsample_mode="bilinear", merge_mode="add"
    )
