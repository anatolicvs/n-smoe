...
class MedicalDatasetSR(torch.utils.data.Dataset):
    def __init__(self, opt):
        ...
        self.use_dataset_cache = opt["use_dataset_cache"] if "use_dataset_cache" in opt else True
        self.dataset_cache_file = opt["dataset_cache_file"] if "dataset_cache_file" in opt else "dataset_cache.pkl"
    ...
    def load_sample(self, fname):
        try:
            ...
            if fname.endswith('.h5'):
                ...
                return [FastMRIRawDataSample(fname, slice_ind, metadata) for slice_ind in range(num_slices)]
    ...
    def load_image_data(self, fname, slice_ind):
        try:
            if fname.endswith('.h5'):
                with h5py.File(fname, "r") as hf:
                    img = hf[self.recons_key][slice_ind] if self.recons_key in hf else None
    ...
    def apply_degradation(self, img, fname):
        ...
        img_L, img_H = {
            ...
            'dpsr': lambda x: blindsr.dpsr_degradation(x, self.k['kernels'][0][1], self.sf),
            'bsrgan_plus': lambda x: blindsr.degradation_bsrgan_plus(x, self.sf, self.lq_patchsize),
        }[chosen_model](img)
