...
flags.DEFINE_integer(
    "inference_generations", 4096, "Number of generations for inference."
)
...
def do_inference(
    ...
    with jax.spmd_mode("allow_all"):
        ...
        if FLAGS.mode == "interpolate":
            ...
            i_neg = np.sqrt(1 - i**2)
            x = (
                eps0[None] * i_neg[:, None, None, None]
        ...
        for fid_it in tqdm.tqdm(range(num_generations // FLAGS.batch_size)):
            ...
            for ti in range(denoise_timesteps):
                ...
                if FLAGS.model.train_type == "naive":
                    dt_flow = np.log2(FLAGS.model["denoise_timesteps"]).astype(
                        jnp.int32
                    )
                    dt_base = (
                        jnp.ones(images_shape[0], dtype=jnp.int32) * dt_flow
                    )  # Smallest dt.
                else:  # shortcut
                ...
                if FLAGS.model.train_type == "consistency":
                    eps = shard_data(
                        jax.random.normal(jax.random.fold_in(eps_key, ti), images_shape)
                    )
                    x1pred = x + v * (1 - t)
