...
FLAGS = flags.FLAGS
flags.DEFINE_string("dataset_name", "celebahq256", "Environment name.")
...
wandb_config.update(
    {
        "project": "straightflow",
        "name": "straightflow_{dataset_name}",
    }
)
...
def main(_):
    ...
    dit_args = {
        ...
        "ignore_dt": (
            False if (FLAGS.model["train_type"] in ("shortcut", "livereflow")) else True
        ),
    }
    ...
    def init(rng):
        ...
        params = model_def.init(
            model_rngs, example_obs, example_t, example_dt, example_label
        )["params"]
    ...
    if (
        FLAGS.model.train_type == "progressive"
        or FLAGS.model.train_type == "consistency-distillation"
    ):
        train_state_teacher = jax.jit(lambda x: x, out_shardings=train_state_sharding)(
            train_state
        )
    else:
    ...
    def update(
        train_state, train_state_teacher, images, labels, force_t=-1, force_dt=-1
    ):
        ...
        if FLAGS.model["cfg_scale"] == 0:  # For unconditional generation.
            labels = (
                jnp.ones(labels.shape[0], dtype=jnp.int32) * FLAGS.model["num_classes"]
            )
        ...
        elif FLAGS.model["train_type"] == "diffusion":
            from targets_diffusion import get_targets
        ...
        def loss_fn(grad_params):
            ...
            if (
                FLAGS.model["train_type"] == "shortcut"
                or FLAGS.model["train_type"] == "livereflow"
            ):
    ...
    # Train Loop
    ###################################
    # bst_dt, bst_t, bst_xt, bst_v, bst_l = None, None, None, None, None

    for i in tqdm.tqdm(
        range(1 + start_step, FLAGS.max_steps + 1 + start_step),
        smoothing=0.1,
        dynamic_ncols=True,
    ):
        ...
        if FLAGS.model["train_type"] == "progressive":
            ...
            if i % (FLAGS.max_steps // num_sections) == 0:
                train_state_teacher = jax.jit(
                    lambda x: x, out_shardings=train_state_sharding
                )(train_state)

        if (
            i % FLAGS.eval_interval == 0
            or (i - start_step) == 100
            or (i == FLAGS.max_steps + start_step)
        ):
