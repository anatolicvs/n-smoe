...
def eval_model(
    ...
    with jax.spmd_mode('allow_all'):
        ...
        for d in d_list:
            infos = None
            # for t in np.arange(0, FLAGS.model.denoise_timesteps):
            ...
            # axs[0, d].set_ylim(0, 1.5)
            if FLAGS.model['train_type'] == 'shortcut':
                axs[1, d].plot(time_axis, infos['loss_flow'])
                axs[1, d].set_title(f"Flow {d}")
                # axs[1, d].set_ylim(0, 1.5)
                axs[2, d].plot(time_axis, infos['loss_bootstrap'])
                axs[2, d].set_title(f"Bootstrap {d}")
            # axs[2, d].set_ylim(0, 1.5)
            # axs[3, d].plot(time_axis, infos['v_magnitude_shortcut'])
            # axs[3, d].set_title(f"Shortcut Mag {d}")

            # axs[4, d].plot(time_axis, infos['v_magnitude_bootstrap'])
            # axs[4, d].set_title(f"Shortcut Targ. {d}")
            # axs[3, d].set_ylim(0, 1.5)
            # axs[3, d].plot(time_axis, infos['flow_scale'])
            # axs[3, d].set_title(f"F/B Ratio {d}")
            # axs[3, d].set_ylim(-0.1, 1.1)
        ...
        for dt_type in ['flow', 'shortcut']:
            if len(jax.local_devices()) == 8:
                ...
                if FLAGS.model.train_type == 'diffusion':
                    from targets_diffusion import GaussianDiffusion
                    scheduler = GaussianDiffusion(FLAGS.model.denoise_timesteps)
                    t_int = FLAGS.model.denoise_timesteps - (t * FLAGS.model.denoise_timesteps).astype(jnp.int32)
                    x_1_pred = scheduler._predict_xstart_from_eps(x_t=x_t, t=t_int, eps=v_pred)
        ...
        for denoise_timesteps in denoise_timesteps_list:
            ...
            for ti in range(denoise_timesteps):
                ...
                if FLAGS.model.train_type != 'diffusion':
                    ...
                    x = scheduler.p_ddim(x, int(FLAGS.model.denoise_timesteps * (1 - t)), v)
        ...
        def do_fid_calc(cfg_scale, denoise_timesteps):
            ...
            for fid_it in tqdm.tqdm(range(num_generations // FLAGS.batch_size)):
                ...
                # if jax.process_index() == 0:
                #     x = np.array(x)
                #     np.save(f'generations/{FLAGS.wandb.name}_{fid_it}.npy', x)
