...
def get_dataset(dataset_name, batch_size, is_train, debug_overfit=False):
    ...
    if "imagenet256" in dataset_name:
        ...
        split = tfds.split_for_jax_process(
            "train" if (is_train or debug_overfit) else "validation",
            drop_remainder=True,
        )
        ...
        dataset = dataset.shuffle(
            20000, seed=42 + jax.process_index(), reshuffle_each_iteration=True
        )
        dataset = dataset.repeat()
        dataset = dataset.batch(batch_size)
        dataset = dataset.prefetch(tf.data.AUTOTUNE)
        ...
        split = tfds.split_for_jax_process(
            "church-train" if is_train else "church-test", drop_remainder=True
        )
